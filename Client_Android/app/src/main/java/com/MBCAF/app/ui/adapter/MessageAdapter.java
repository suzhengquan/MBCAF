...
import com.MBCAF.app.ui.activity.PreviewGifActivity;
import com.MBCAF.app.ui.activity.PreviewMessageImagesActivity;
import com.MBCAF.app.ui.activity.PreviewTextActivity;
import com.MBCAF.app.ui.helper.Emoparser;
import com.MBCAF.app.ui.widget.GifView;
import com.MBCAF.app.ui.widget.message.GifImageRenderView;
...
import com.MBCAF.app.ui.widget.SpeekerToast;
import com.MBCAF.app.ui.widget.message.AudioRenderView;
import com.MBCAF.app.ui.widget.message.EmojiRenderView;
import com.MBCAF.app.ui.widget.message.ImageRenderView;
import com.MBCAF.app.ui.widget.message.MessageOperatePopup;
import com.MBCAF.app.ui.widget.message.RenderType;
import com.MBCAF.app.ui.widget.message.TextRenderView;
import com.MBCAF.app.ui.widget.message.TimeRenderView;
import com.MBCAF.app.ui.helper.listener.OnDoubleClickListener;
...
public class MessageAdapter extends BaseAdapter {
    ...
    public void addItem(final MessageEntity msg) {
        if (msg.getDisplayType() == PreDefine.MT_Text) {
            if (isMsgGif(msg)) {
                msg.setGIfEmo(true);
            } else {
                msg.setGIfEmo(false);
        ...
        /**消息的判断*/
        if (msg.getDisplayType() == PreDefine.View_Mix_Type) {
...
//                    imageMessage.setLoadStatus(MessageStatus.IMAGE_UNLOAD);//如果是图片已经上传成功呢？
                    ...
                    Toast.makeText(ctx, ctx.getString(R.string.t052), Toast.LENGTH_LONG).show();
                }
            }
            ...
            public void onMsgSuccess() {
                ...
                bundle.putSerializable(PreDefine.CUR_MESSAGE, imageMessage);
                i.putExtras(bundle);
                ctx.startActivity(i);
...
//                imageMessage.setPath(loaclPath);//下载的本地路径不再存储
        ...
        messageImage.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                boolean bResend = (imageMessage.getStatus() == PreDefine.MSG_FAILURE)
                        || (imageMessage.getLoadStatus() == PreDefine.IMAGE_UNLOAD);
                popup.show(messageLayout, PreDefine.View_Image_Type, bResend, isMine);
                return true;
            }
        });
        ...
        imageRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                // 重发或者重新加载
                MessageOperatePopup popup = getPopMenu(parent, new OperateItemClickListener(imageMessage, position));
                popup.show(messageLayout, PreDefine.View_Image_Type, true, isMine);
            }
        });
    ...
    private View GifImageMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ...
        imageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                intent.putExtra(PreDefine.PREVIEW_TEXT_CONTENT, url);
    ...
    private View audioMsgRender(final int position, View convertView, final ViewGroup parent, final boolean isMine) {
        ...
        if (!TextUtils.isEmpty(audioPath)) {
            ...
            messageLayout.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    ...
                    boolean bResend = audioMessage.getStatus() == PreDefine.MSG_FAILURE;
                    popup.show(messageLayout, PreDefine.View_Audio_Type, bResend, isMine);
                    return true;
                }
            });
        }
        ...
        audioRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                MessageOperatePopup popup = getPopMenu(parent, new OperateItemClickListener(audioMessage, position));
                popup.show(messageLayout, PreDefine.View_Audio_Type, true, isMine);
            }
        });
    ...
    private View textMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        ...
        textRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                popup.show(textView, PreDefine.View_Text_Type, true, isMine);
            }
        });
        ...
        textView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                // 弹窗类型
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(textMessage, position));
                boolean bResend = textMessage.getStatus() == PreDefine.MSG_FAILURE;
                popup.show(textView, PreDefine.View_Text_Type, bResend, isMine);
                return true;
            }
        });
        ...
        textView.setOnTouchListener(new OnDoubleClickListener() {
            ...
            public void onDoubleClick(View view) {
                ...
                intent.putExtra(PreDefine.PREVIEW_TEXT_CONTENT, content);
                ctx.startActivity(intent);
            }
        });
    ...
    private View gifMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        ...
        gifRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(textMessage, position));
                popup.show(imageView, PreDefine.View_AniImage_Type, true, isMine);
            }
        });
        ...
        imageView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(textMessage, position));
                boolean bResend = textMessage.getStatus() == PreDefine.MSG_FAILURE;
                popup.show(imageView, PreDefine.View_AniImage_Type, bResend, isMine);
    ...
    private class OperateItemClickListener
        ...
        public void onResendClick() {
            try {
                if (mType == PreDefine.View_Audio_Type
                        || mType == PreDefine.View_Text_Type) {

                    if (mMsgInfo.getDisplayType() == PreDefine.View_Audio_Type) {
                        if (mMsgInfo.getSendContent().length < 4) {
                            return;
                        }
                    }
                } else if (mType == PreDefine.View_Image_Type) {
                    logger.d("pic#resend");
                    // 之前的状态是什么 上传没有成功继续上传
                    // 上传成功，发送消息
                    ImageMessage imageMessage = (ImageMessage) mMsgInfo;
                    if (TextUtils.isEmpty(imageMessage.getPath())) {
                        Toast.makeText(ctx, ctx.getString(R.string.t053), Toast.LENGTH_LONG).show();
                        return;
                    }
                }
                mMsgInfo.setStatus(PreDefine.MSG_SENDING);
        ...
        public void onSpeakerClick() {
            ...
            if (audioPlayerHandler.getAudioMode(ctx) == AudioManager.MODE_NORMAL) {
                ...
                SpeekerToast.show(ctx, ctx.getText(R.string.t032), Toast.LENGTH_SHORT);
            } else {
                audioPlayerHandler.setAudioMode(AudioManager.MODE_NORMAL, ctx);
                SpeekerToast.show(ctx, ctx.getText(R.string.t031), Toast.LENGTH_SHORT);
            }
        }
    }
}
