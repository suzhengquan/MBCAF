...
import com.MBCAF.pb.base.DataBuffer;
...
public class IMSocketManager extends IMManager 
{
    ...
    private IMPacketManager listenerQueue = IMPacketManager.instance();
    ...
    public void sendRequest(GeneratedMessageV3 requset, int sid, int cid, IMPacketManager.PacketListener packetlistener)
    {
        ...
        {
            //组装包头 header
            com.MBCAF.pb.base.Header header = new com.MBCAF.pb.base.Header(sid, cid);
            int bodySize = requset.getSerializedSize();
            header.setLength(PreDefine.PROTOCOL_HEADER_LENGTH + bodySize);
    ...
    public void packetDispatch(ChannelBuffer channelBuffer){
        ...
        com.MBCAF.pb.base.Header header = new com.MBCAF.pb.base.Header();
        header.decode(buffer);
        /**buffer 的指针位于body的地方*/
        int commandId = header.getMessageID();
        int serviceId = header.getServiceId();
        int seqNo = header.getSeqnum();
        logger.d("dispatch packet, serviceId:%d, commandId:%d", serviceId, commandId);
        
        CodedInputStream codedInputStream = CodedInputStream.newInstance(new ChannelBufferInputStream(buffer.getOrignalBuffer()));

       IMPacketManager.PacketListener listener = listenerQueue.pop(seqNo);
       if(listener!=null)
       {
        ...
        switch (serviceId)
        {
            case Proto.MsgTypeID.MTID_ServerBase_VALUE:
                IMPacketDispatcher.loginPacketDispatcher(commandId,codedInputStream);
                break;
            case Proto.MsgTypeID.MTID_MsgServer_VALUE:
                IMPacketDispatcher.msgPacketDispatcher(commandId,codedInputStream);
                break;
            default:
                logger.e("packet#unhandled serviceId:%d, commandId:%d", serviceId, commandId);
                break;
        }
    }
