...
public class IMGroupAvatar extends LinearLayout {
    ...
    public IMGroupAvatar(Context context, AttributeSet attrs) {
        ...
        parentPadding = SysUtil.instance(getContext()).dip2px(DEFAULT_PADDING_DIP);
        childMargin = SysUtil.instance(getContext()).dip2px(DEFAULT_MARGIN_DIP);
        parentAvatarSize = SysUtil.instance(getContext()).dip2px(DEFAULT_VIEW_SIZE_DIP);
        setPadding(parentPadding, parentPadding, parentPadding, parentPadding);
    }

    public void setParentPadding(int padding){
        if (padding >= 0) {
            parentPadding = SysUtil.instance(getContext()).dip2px(padding);
            setPadding(parentPadding, parentPadding, parentPadding, parentPadding);
        }
    }
    ...
    private void initOneRow(int curRow,boolean isFirst, int columnSize){
        try {
            ...
            for (int i = 0; i < columnSize; i++) {
                ...
                avatar.setCorner(SysUtil.instance(getContext()).dip2px(childCorner));
